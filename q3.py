...
class Puzzle():
    ...
    def CheckforMatchWithPattern(self, Row, Column):
        LeftBound = max(0, Column - 2)#
        RightBound = min(self.__GridSize, Column + 3)#
        for StartRow in range(Row + 2, Row - 1, -1):
            TopBound = max(0, StartRow - 2)#
            BottomBound = min(self.__GridSize, StartRow + 1)#
            for StartColumn in range(LeftBound, RightBound):#
                try:
                    PatternString = ""
                    PatternString += self.__GetCell(StartRow, StartColumn % self.__GridSize).GetSymbol()
                    PatternString += self.__GetCell(StartRow, (StartColumn + 1) % self.__GridSize).GetSymbol()#
                    PatternString += self.__GetCell(StartRow, (StartColumn + 2) % self.__GridSize).GetSymbol()#
                    PatternString += self.__GetCell(StartRow - 1, (StartColumn + 2) % self.__GridSize).GetSymbol()#
                    PatternString += self.__GetCell(StartRow - 2, (StartColumn + 2) % self.__GridSize).GetSymbol()#
                    PatternString += self.__GetCell(StartRow - 2, (StartColumn + 1) % self.__GridSize).GetSymbol()#
                    PatternString += self.__GetCell(StartRow - 2, (StartColumn) % self.__GridSize).GetSymbol()#
                    PatternString += self.__GetCell(StartRow - 1, (StartColumn) % self.__GridSize).GetSymbol()#
                    PatternString += self.__GetCell(StartRow - 1, (StartColumn + 1) % self.__GridSize).GetSymbol()#

                    for P in self.__AllowedPatterns:
                        CurrentSymbol = self.__GetCell(Row, Column).GetSymbol()
                        if P.MatchesPattern(PatternString, CurrentSymbol):
                            self.__GetCell(StartRow, StartColumn).AddToNotAllowedSymbols(CurrentSymbol)#
                            self.__GetCell(StartRow, (StartColumn + 1) % self.__GridSize).AddToNotAllowedSymbols(CurrentSymbol)#
                            self.__GetCell(StartRow, (StartColumn + 2) % self.__GridSize).AddToNotAllowedSymbols(CurrentSymbol)#
                            self.__GetCell(StartRow - 1, (StartColumn + 2) % self.__GridSize).AddToNotAllowedSymbols(CurrentSymbol)#
                            self.__GetCell(StartRow - 2, (StartColumn + 2) % self.__GridSize).AddToNotAllowedSymbols(CurrentSymbol)#
                            self.__GetCell(StartRow - 2, (StartColumn + 1) % self.__GridSize).AddToNotAllowedSymbols(CurrentSymbol)#
                            self.__GetCell(StartRow - 2, (StartColumn) % self.__GridSize).AddToNotAllowedSymbols(CurrentSymbol)#
                            self.__GetCell(StartRow - 1, (StartColumn) % self.__GridSize).AddToNotAllowedSymbols(CurrentSymbol)#
                            self.__GetCell(StartRow - 1, (StartColumn + 1) % self.__GridSize).AddToNotAllowedSymbols(CurrentSymbol)#
                            return 10
                except:
                    pass
        return 0
