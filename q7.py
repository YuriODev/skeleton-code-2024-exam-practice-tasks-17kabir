...
def Main():
    ...
    while Again == "y":
        ...
        if len(Filename) > 0:
            ...
            MyPuzzle = Puzzle(8, int(8 * 8 * 0.6)) #60% of the grid is full, puzzle finished
    ...
    def __init__(self, *args):
        if len(args) == 1:
            ...
            for Count in range(1, self.__GridSize * self.__GridSize + 1):
                if random.randrange(1, 101) < 90: #10% chance of getting a puzzle letter in a given cell
                    C = Cell()
                else:
                    C = BlockedCell()
            ...
            self.__AllowedPatterns = []
            self.__AllowedSymbols = []
            QPattern = Pattern("Q", "QQ**Q**QQ") #*'s are filler no.s
            ...
            TPattern = Pattern("T", "TTT**T**T")#indexes: ("1","2,3,4,5,6,7,8,9")
            self.__AllowedPatterns.append(TPattern) #123894765 are the actual indexes
            self.__AllowedSymbols.append("T") #it goes in a spiral
    ...
    def CheckforMatchWithPattern(self, Row, Column):
        for StartRow in range(Row + 2, Row - 1, -1):
            for StartColumn in range(Column - 2, Column + 1):
                try:#collects all of the cells in a given 3x3 and stores them as a string
                    PatternString = ""#startrow and startcolumn is what you entered
                    ...
                    for P in self.__AllowedPatterns:#checks if the cells in a given 3x3 can score
                        CurrentSymbol = self.__GetCell(Row, Column).GetSymbol() #the symbol you just entered
...
class Cell():#makes the cell
    def __init__(self):
        self._Symbol = ""
        self.__SymbolsNotAllowed = []

    def GetSymbol(self): #returns the symbol (if there is any)
        if self.IsEmpty():
          return "-"
        else:
          return self._Symbol

    def IsEmpty(self): #returns if the cell is empty or not
        if len(self._Symbol) == 0:
            return True
        else:
            return False

    def ChangeSymbolInCell(self, NewSymbol): #adds/changes symbols in a cell
        self._Symbol = NewSymbol

    def CheckSymbolAllowed(self, SymbolToCheck):#checks if a cell can be changed
        for Item in self.__SymbolsNotAllowed:
            if Item == SymbolToCheck:
                return False
        return True

    def AddToNotAllowedSymbols(self, SymbolToAdd): #adds to the array self.__SymbolsNotAllowed
        self.__SymbolsNotAllowed.append(SymbolToAdd)

    def UpdateCell(self): #useless atm
        pass

class BlockedCell(Cell): 
    def __init__(self):
        super(BlockedCell, self).__init__() #goes to the constructor and collects the values there
        self._Symbol = "@" #symbol to show that a cell cannot be changed here

    def CheckSymbolAllowed(self, SymbolToCheck):#checks if u can use a symbol in that cell
        return False
